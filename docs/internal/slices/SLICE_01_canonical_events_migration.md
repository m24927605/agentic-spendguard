...
Layered on top of `docs/internal/review-standards/predictor-review-checklist.md` §1:
...
- **Rollback order** (per round-2 fix M16 cross-DB ordering + round-3 B2/B3 + round-4 M4/M5/B1 updates):
  ...
  2. For EACH down-migration file you intend to apply, opt in to the per-file destructive guard on the target Postgres session (round-4 fix M4 — per-file GUC scope replaces round-3's shared `spendguard.allow_destructive_down`; round-5 fix N12-A — `SET`, not `SET LOCAL`, see rationale below). The exact form is:
     ```sql
     -- Canonical DB session (apply both before 0015_down + 0013_down):
     SET spendguard.allow_destructive_down_0015 = 'on';
     SET spendguard.allow_destructive_down_0013 = 'on';
     -- Ledger DB session (apply both before 0048_down + 0046_down):
     SET spendguard.allow_destructive_down_0048 = 'on';
     SET spendguard.allow_destructive_down_0046 = 'on';
     ```
     Round-5 N12-A: `SET` (session-scoped), not `SET LOCAL` (transaction-scoped). The migration runner (`charts/spendguard/templates/migrations.yaml` and `deploy/demo/init/migrations/{10,20}_apply_*.sh`) invokes `psql` WITHOUT `--single-transaction`, so every top-level statement autocommits — including a single `SET LOCAL` line. The GUC would die at that autocommit boundary before the next DO block runs and the destructive-down guard would fire as if the operator never opted in. `SET` persists for the entire psql session, which is what we want when the operator runs `\i <down_file>` one file at a time in psql. The single-quote `'on'` form is required because the GUC value is parsed as a string, not a boolean literal. Without the matching GUC, each down-migration file aborts on its first statement with `destructive down-migration NNNN requires SET spendguard.allow_destructive_down_NNNN = on first`. Each file also emits `NOTICE: DESTRUCTIVE down-migration NNNN proceeding (caller: <role>)` so PG audit logs capture the rollback (round-4 M4). After running the file, the operator SHOULD `RESET spendguard.allow_destructive_down_NNNN;` to revoke the opt-in for any subsequent commands in the same session — best-practice hygiene, not a correctness requirement (each file checks its own per-file GUC; cross-file leakage cannot accidentally enable a different file's destructive path).
...
- Recommended reviewer profile: Database Optimizer（per HANDOFF §10.1）or `Backend Architect`
- Review depth: deep（migration changes are high-stakes）
...
## §14. Merge checklist (before merge)
...
- [ ] All 8+ demo modes 全綠
- [ ] commit metadata references review evidence when available
