...
- `docs/internal/reviews/hardening/HARDEN_02/e2e-runbook.md`
- `docs/internal/reviews/hardening/HARDEN_02/command-results.md`
- `docs/internal/reviews/hardening/HARDEN_02/kind-production-values.example.yaml`
- `docs/internal/reviews/hardening/HARDEN_02/benchmark-results.json`
...
- `helm template charts/spendguard --set chart.profile=production -f docs/internal/reviews/hardening/HARDEN_02/kind-production-values.example.yaml` passes
...
- Results are committed under `docs/internal/reviews/hardening/HARDEN_02/`
...
The reviewer must inspect `docs/internal/reviews/hardening/HARDEN_02/command-results.md` and reject the slice if demo commands were not actually run.
...
| Codex R1 | codex adversarial reviewer `review_01KSXMST512E6H23FEST1VGJE3` | Fix all findings in-slice | Closed proxy double-advance, untrusted budget metadata, and denied mirror gaps |
| Codex R2 | codex adversarial reviewer `review_01KSXNXYHSRM7JBWP93B9W6TE0` | Fix all findings in-slice | Closed projector idempotency, authoritative audit field, and production auto-wiring issues |
| Codex R3 | codex adversarial reviewer `review_01KSXQ6WGVBHEPD276771DW4GN` | Fix all findings in-slice | Closed production budget source, projector concurrency, and policy override issues |
| Codex R4 | codex adversarial reviewer `review_01KSXRBRK7H72G9A5NXA9PTP7A` | Fix all findings in-slice | Closed ledger balance double-counting and durable decision replay after cache loss |
| Codex R5 | codex adversarial reviewer `review_01KSXT8P6C2XT2Y2Z4G5PEJ1FD` | Escalate after max codex rounds | Two high findings remained: replay request validation and production example projector wiring |
