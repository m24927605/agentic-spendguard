...
| AIT Round 3 found Strategy C P95 1.31-1.50 downgraded to the generic warning formatter label even though Rule 4 classifies C P95 > 1.05 as critical under-prediction. | Fixed text and markdown formatter ordering so Strategy C under-prediction beats generic warning labels. Added regressions for C P95 1.31. `cargo test --manifest-path services/calibration_report/Cargo.toml` passed with 99 unit tests, 7 CLI smoke tests, 3 sample-output tests, and 13 scenario tests. |
