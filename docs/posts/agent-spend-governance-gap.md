...
- A small group of us are publishing a draft wire protocol — the [Agent Spend Protocol (ASP) Draft-01](https://github.com/m24927605/agentic-spendguard/blob/main/docs/specs/agent-spend-protocol/draft-01.md). Apache 2.0, public-domain spec sketch. Reserve / Commit / Audit semantics, signature discipline, transaction model. Not bound to any one implementation.
- We're also proposing [an OpenTelemetry GenAI extension](https://github.com/m24927605/agentic-spendguard/blob/main/docs/proposals/otel-genai-spend-extension.md) so spend-decision events surface on the same GenAI span as the provider call itself. Existing OTel dashboards keep working; spend governance becomes just another span event.
- And we're aligning terminology with Tymofii Pidlisnyi's [`agent-governance-vocabulary`](https://github.com/aeoess/agent-governance-vocabulary) — the multi-vendor neutral-ground vocab that already powers crosswalks for APS, SINT, AgentID, AgentGraph, MolTrust, and ScopeBlind. A formal crosswalk for ASP terms is on the way.
