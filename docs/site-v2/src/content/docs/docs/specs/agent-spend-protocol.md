---
title: "Agent Spend Protocol — Draft 01"
description: "Wire-level contract for pre-call budget reservation, post-call usage reconciliation, and signed audit emission. SpendGuard binding of the upstream budget_reservation canonical-verb crosswalk."
---
...
**Key management.** Every CloudEvent envelope MUST carry a `kid` (signing key identifier) **inside the `data` payload** so it is covered by the signed scope above. Putting `kid` in a CloudEvent extension attribute is NOT permitted, because extension attributes are not in the signed-scope field list and a relay could rewrite or strip the key identifier without invalidating the signature. The issuer MUST publish a JWKS document at a well-known URL discoverable from the issuer's domain. After key rotation, **previous verification keys MUST remain published** for at least the retention period of the audit chain they signed (RECOMMENDED ≥ 1 year). Without this, historical audit chains become unverifiable.
...
- **OpenTelemetry GenAI** — ASP composes with OTel GenAI by emitting span events on the GenAI span. The event names follow the [parallel OTel SIG proposal](/docs/specs/otel-genai-extension/): `gen_ai.spend.reserve`, `gen_ai.spend.commit`, `gen_ai.spend.release`, `gen_ai.spend.audit`. Earlier drafts of this spec used the bare `asp.*` prefix; that has been retired in favor of the OTel-aligned names.
...
[SpendGuard](https://github.com/m24927605/agentic-spendguard) (Apache-2.0) is a **partial reference implementation** of Draft-01 — partial meaning it implements the protocol's transaction model and audit chain but with three known deltas from the wire shape above (down from four at Draft-01 publication; the Release wire shape delta is closed). Both directions of work (spec revision to match SpendGuard, or SpendGuard revision to match spec) are in scope for Draft-02.
...
- **Release wire shape (closed 2026-05-23).** SpendGuard now exposes an explicit `ReleaseReservation` RPC on the adapter UDS surface matching Draft-01 §4 (`reservation_id` / `idempotency_key` / `reason_codes`). It wraps the same `transaction::run_release` core that the implicit outcome-driven release paths (`ConfirmPublishOutcome.APPLY_FAILED`, `EmitTraceEvents` run-aborted) use; both paths coexist. Reason-code mapping (aligned with the EmitTraceEvents implicit path so audit reason values are consistent regardless of which release path the adapter took): `provider_error` / `runtime_error` / `client_timeout` → `RuntimeError`; `run_aborted` / `run_cancelled` → `RunAborted`; anything else → `Explicit`.

**Known v1 limitations** (tracked as GitHub issues, not blockers for the delta closure):

- Replay branch returns empty `audit_event_signature` instead of the original (sidecar would need to cache emitted signatures or the ledger would need to store + return them) — [GH #85](https://github.com/m24927605/agentic-spendguard/issues/85)
- `ReleaseReservation` retries fail with `FENCING_EPOCH_STALE` if the sidecar's fencing lease changed since the original call — the preflight `check_active` blocks replay before `run_release` can detect it — [GH #86](https://github.com/m24927605/agentic-spendguard/issues/86)
- Ledger `IdempotencyConflict` surfaces as gRPC `INTERNAL` instead of `FailedPrecondition`; needs a dedicated proto error code on the ledger side — [GH #87](https://github.com/m24927605/agentic-spendguard/issues/87) The richer ledger-tier `ReleaseRequest` (with `reservation_set_id`, `Idempotency`, `Fencing`, `audit_event`, `decision_id`, `producer_sequence`) stays internal to the sidecar; adapters see only the Draft-01-shaped wire surface. See proto: `proto/spendguard/sidecar_adapter/v1/adapter.proto::ReleaseReservation`.
