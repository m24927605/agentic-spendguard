...
`prost` v0.x default behavior：unknown fields 在 decode 時被**保留**（轉成 unknown wire bytes），re-encode 時**保留原 wire bytes**。所以舊 verifier 即便不認識 300-317 fields，re-encode 仍 byte-identical → 簽章驗證通過。

驗證：SLICE 01 acceptance 必含 「prost::Message round-trip preservation」property test。
