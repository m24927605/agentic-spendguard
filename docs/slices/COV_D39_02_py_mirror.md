...
|---|---|---|---|
| `ag-ui-protocol` latest version / extra range / test pin | Latest PyPI release is **0.1.19** (release line 0.1.4-0.1.19, no 0.2.x) — identical to the spec baseline. Extra stays `ag-ui = ["ag-ui-protocol>=0.1.19,<0.2"]`; exact test pin `ag-ui-protocol==0.1.19` added to the `dev` dependency group | **PRIMARY** — spec baseline confirmed, no drift | `https://pypi.org/pypi/ag-ui-protocol/json` queried 2026-06-10 (`info.version == 0.1.19`). Pins live in `sdk/python/pyproject.toml` (`ag-ui` extra + `dev` group comment). |
| Python `CustomEvent` import path | `ag_ui.core.CustomEvent` (defined in `ag_ui.core.events`, re-exported from `ag_ui.core`), a pydantic `BaseModel` with fields `{type, timestamp, raw_event, name, value}`; `CustomEvent.model_validate(built_event)` succeeds with and without the optional `timestamp` → TA-28 uses the PRIMARY runtime-parse path, NOT the TP-29 key-set fallback | **PRIMARY** | Inspected installed wheel `ag-ui-protocol==0.1.19` (`ag_ui/core/events.py`); TA-28 runs 5/5 green under the exact pin. Recorded in `tests/integrations/ag_ui/test_ag_ui_compat.py` header docstring. |
| `DecisionStopped` STOP vs STOP_RUN_PROJECTION — Python err-class half | The Python SDK error class does **NOT** expose the wire decision enum as a structured attribute: `DecisionStopped` (`sdk/python/src/spendguard/errors.py:74`) carries only `decision_id` / `reason_codes` / `audit_decision_event_id` / `matched_rule_ids`; the raising site (`client.py:545-552`) collapses both `STOP` and `STOP_RUN_PROJECTION` into one class, with the wire name appearing only inside the human-readable message string (not a contract) | **FALLBACK** (pre-declared §5.7 text): callers emit `denied_kind="STOP"`; the projection nuance stays visible in `reason_codes` (`RUN_BUDGET_PROJECTION_EXCEEDED`, `RUN_STEPS_EXCEEDED`, `RUN_CEILING`, `RUN_DRIFT_DETECTED`). The §5.7 `denied_kind` enum is UNCHANGED — `STOP_RUN_PROJECTION` remains a legal builder input for callers that know the wire value by other means (slice 3 owns the demo-mapping half) | Source inspection of `errors.py` + `client.py` at impl time; no code change made either way per the marker instruction. |
