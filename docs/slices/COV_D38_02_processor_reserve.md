...
|---|---|---|---|
| V1 | Exact `Processor` hook signatures (args object shape, async contract) for `processInputStep` / `processLLMRequest` / `processLLMResponse` / `processOutputStep` | n/a — `implements Processor` typecheck is the gate; doc records shapes | PINNED 2026-06-10 vs `@mastra/core@1.41.0` — see "Marker resolutions" below |
| V2 | Does a throw from `processInputStep` halt the step pre-provider, or is the hook-provided `abort()` (TripWire) required? | throw directly / call abort() with typed error on `cause` — observable contract fixed either way (§7.3) | PINNED: **throw directly** (TP-10 empirical) — see "Marker resolutions" below |
| V3 | Is a stable per-call/per-step correlation id visible at both reserve and commit hooks? | key inflight by that id / LOCKED per-runId FIFO fallback (§6.5) | PINNED: **no shared id → LOCKED per-runId FIFO fallback** — see "Marker resolutions" below |
| V5 | Exact Agent constructor key for mounting processors in `@mastra/core` 1.x (`inputProcessors`/`outputProcessors`/unified list) | record + use the installed key; quickstart copies it | PINNED: **`inputProcessors`** — see "Marker resolutions" below |

### Marker resolutions (recorded at impl, 2026-06-10, installed `@mastra/core@1.41.0`)

| ID | Found (installed reality) | Resolution (primary / pre-declared fallback) |
|---|---|---|
| V1 | `Processor<TId, TTripwireMetadata>` REQUIRES `readonly id: TId`; `name` is optional — the §5 shell shows only `name`, but the §5 "typechecks against the installed peer" rule mandates `id` too (both carry `"spendguard-processor"`). Agent config ALSO requires `id` alongside `name`. `processInputStep(args: ProcessInputStepArgs)` — args extend `ProcessorMessageContext`: `messages: MastraDBMessage[]` (`{id, role, createdAt, content: {format: 2, parts}}`, text parts `{type:"text", text}`), `messageList`, `stepNumber` (0-indexed), `steps`, `systemMessages` (instructions travel here, NOT on `messages`), `state`, `model`, `abort(reason?, opts?): never`, `retryCount`, optional `messageId`/`requestContext`/`writer`/`abortSignal`; returning `undefined` = "no changes". `processLLMRequest(args: ProcessLLMRequestArgs)` → `ProcessLLMRequestResult \| undefined`. Hooks may be sync or async. | n/a — `implements Processor` typecheck is the gate (TP-02); shapes recorded in `src/processor.ts` header + here |
| V2 | A plain throw from `processInputStep` halts the step BEFORE the provider call (stub model records 0 `doGenerate`/`doStream`) and `agent.generate()` REJECTS. `abort()` (TripWire) is NOT required — and is unusable: a TripWire makes `generate()` RESOLVE with a tripwire result (no rejection). Honest limitation: Mastra 1.41.0 wraps input processors in an internal workflow that SERIALIZES step errors — the consumer-facing rejection preserves the typed error's MESSAGE but not the class instance (`cause` chain ends in a serialized POJO). The typed error holds `instanceof` at the hook boundary (TP-13..16) and the observable contract (DENY ⇒ zero provider calls, Agent rejects) is fully met (TP-10). | **Primary: throw directly** (first pre-declared alternative) |
| V3 | NO stable per-call/per-step correlation id is visible at both `processInputStep` and `processLLMResponse`/`processOutputStep`: the hooks share only `stepNumber` (request-scoped, not unique across concurrent runs) and the per-processor `state` bag; no Mastra run id is exposed at the hook surface. | **LOCKED fallback: per-`runId` FIFO queue** (§6.5) — `InflightMap` keyed by the adapter-derived `runId`; real-loop evidence: `runId === llmCallId` on the wire when no `runIdProvider` is set |
| V5 | Agent constructor key is **`inputProcessors`** (a separate `outputProcessors` list exists; NO unified list). The agent loop builds its per-request ProcessorRunner from `inputProcessors`, which drives `processInputStep` AND `processLLMRequest`/`processLLMResponse`; `processOutputStep` mounting is recorded by COV_D38_03 (V4/V7). Proven on both a stub-model Agent and a model-router-string Agent (TP-22). | record + use installed key: `new Agent({ id, name, instructions, model, inputProcessors: [guard] })` |

> **V2 residual (recorded 2026-06-10, R2):** the design §7.3 consumer-reachability prong is NOT fully met by the V2 pin — Mastra 1.41.0 serializes step errors inside its internal workflow, so the typed error reaching the consumer at the Agent boundary is **message-only** (the `cause` chain ends in a serialized POJO; `instanceof` holds only at the hook boundary, TP-13..16). Consumer catch contract: message-match at the Agent boundary; `instanceof` at the hook boundary. Tracked as `(gh issue #181: D38 V2 cause-chain)`; `// RESIDUAL(D38-V2)` marker at the throw site in `src/processor.ts`; COV_D38_06 documents it user-facing.
