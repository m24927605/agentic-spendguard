...
> NOTE-TO-ORCHESTRATOR: review-standards.md §"Reviewer prompt template" tells the reviewer to read slice docs at `docs/specs/coverage/D38_mastra/slices/<SLICE_ID>.md`, but per the build directive these slice docs live at `docs/slices/COV_D38_*.md` (D04/D11 convention). Fill the reviewer template with the `docs/slices/` paths or symlink; do not duplicate the docs.
