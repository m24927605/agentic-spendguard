...
| Marker | Where | Pre-declared fallback | **RESOLVED (impl 2026-06-10)** |
|---|---|---|---|
| `reservations` PK column for the ledger join (`<pk_column>` in the Makefile snippet) | design §9.3, impl §8 | None invented — read the actual ledger migration/schema and record the verified column name here. The join itself is non-negotiable (review-standards §7.6). | **`reservation_id`** — `services/ledger/migrations/0010_projections.sql:38` declares `reservation_id UUID PRIMARY KEY` on `CREATE TABLE reservations`. Used verbatim in the `demo-verify-ag-ui-events` psql join; join exercised green against a live run. |
| queryBudget wire status at demo time | design §5.3 | Per the §5.3 marker text: "if the queryBudget wire lands before COV_D39_03, the demo MUST source the snapshot from it instead of seed env vars." Otherwise: seed env values with the ledger cross-check exactly as design §9.2 specifies. Record which path was taken. | **NOT wired** — `sdk/typescript/src/client.ts` `queryBudget()` still throws `QUERY_BUDGET_NOT_YET_WIRED` (§9.4 placeholder); no `QueryBudget` RPC on the adapter wire (`proto/` has only the ledger-internal `QueryBudgetState`). Path taken: seed env values (`SPENDGUARD_DEMO_OPENING_BALANCE_ATOMIC=500`) with the ledger cross-check in `verify_step_ag_ui_events.sql` (asserts the seeded available_budget credit, ledger_entry `…-0030`, equals 500). |
| `DecisionStopped` STOP vs STOP_RUN_PROJECTION distinguishability — **demo-mapping half** (slice 2 owned the Python err-class half) | design §5.7 | The demo's deny step catches `DecisionDenied` → `denied_kind: "DENY"`, so the demo does not depend on the distinction; record the slice-2 finding and per §5.7: if not distinguishable, callers emit `STOP` and the projection nuance stays in `reason_codes`. | **Not distinguishable** — TS `DecisionStopped` (`sdk/typescript/src/errors.ts:143`) exposes no wire decision enum field (matches the slice-2 Python finding: `spendguard/errors.py` `DecisionStopped` likewise carries none). Demo maps by error class name (`DecisionStopped`→`STOP`, `DecisionSkipped`→`SKIP`, `ApprovalRequired`→`APPROVAL_REQUIRED`, base→`DENY`); the live deny is the seeded `hard-cap-deny` STOP rule, so the demo emitted `denied_kind:"STOP"` with the nuance in `reason_codes` (`BUDGET_EXHAUSTED`, `LARGE_CLAIM_REQUIRES_APPROVAL`). |
| AG-UI SSE frame shape (data-only) — **consumed** (resolved by slice 1) | design §7 | Use the slice-1 recorded resolution for demo consumers; `encodeSse` framing (`"data: " + canonical + "\n\n"`) is locked regardless (design §7). | Consumed slice-1 resolution: data-only frames. `verify_sse.py` rejects any non-`data: ` SSE line; live capture round-trips byte-identically under the §7 rule. |
