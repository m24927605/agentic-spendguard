...
| U06 | `test_env_default_resolver_constructs_binding` | With all 8 env vars set, the loaded resolver returns a `BudgetBinding` whose fields equal the env values. **Defer to SLICE 4b (added 2026-06-07 in SLICE 4 R2)**: this test requires resolver-module wiring (`_load_resolver_from_env`) which SLICE 4 deferred. Implement when SLICE 4b lands. SLICE 4 R1 reviewer carved this out of scope per `implementation.md` §4 SLICE-PHASING note. (U06-U10 collectively cover the resolver-module path, the budget-binding env vars, and the 4 pricing-version vars.) |
| U07 | `test_env_default_resolver_loads_unit_ref_and_pricing` | `BudgetBinding.unit.unit_id` and `BudgetBinding.pricing.pricing_version` match env. **Defer to SLICE 4b (added 2026-06-07 in SLICE 4 R2)**: this test requires resolver-module wiring (`_load_resolver_from_env`) and the 4 pricing-version env vars which SLICE 4 deferred. Implement when SLICE 4b lands. SLICE 4 R1 reviewer carved this out of scope per `implementation.md` §4 SLICE-PHASING note. |
| U08 | `test_resolver_module_env_imports_factory` | `SPENDGUARD_RESOLVER_MODULE=tests.fixtures.fake_resolver:make_triple` imports + dispatches to operator factory, ignores single-tenant env vars. **Defer to SLICE 4b (added 2026-06-07 in SLICE 4 R2)**: this test requires the `_load_resolver_from_env` helper and `SPENDGUARD_RESOLVER_MODULE` env var, both deferred to SLICE 4b. Implement when SLICE 4b lands. SLICE 4 R1 reviewer carved this out of scope per `implementation.md` §4 SLICE-PHASING note. |
| U09 | `test_resolver_module_bad_path_raises` | `SPENDGUARD_RESOLVER_MODULE=nonexistent.module:bad` → `SpendGuardConfigError` at boot. **Defer to SLICE 4b (added 2026-06-07 in SLICE 4 R2)**: this test requires `_load_resolver_from_env` import-failure handling which SLICE 4 deferred. Implement when SLICE 4b lands. SLICE 4 R1 reviewer carved this out of scope per `implementation.md` §4 SLICE-PHASING note. |
| U10 | `test_resolver_module_missing_attr_raises` | Module imports but `bad_fn` missing → `SpendGuardConfigError`. **Defer to SLICE 4b (added 2026-06-07 in SLICE 4 R2)**: this test requires `_load_resolver_from_env` attribute-lookup handling which SLICE 4 deferred. Implement when SLICE 4b lands. SLICE 4 R1 reviewer carved this out of scope per `implementation.md` §4 SLICE-PHASING note. |
