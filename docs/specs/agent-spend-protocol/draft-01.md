...
- **Release wire shape (closed 2026-05-23; replay semantics hardened 2026-06-02).** SpendGuard now exposes an explicit `ReleaseReservation` RPC on the adapter UDS surface matching Draft-01 §4 (`reservation_id` / `idempotency_key` / `reason_codes`). It wraps the same `transaction::run_release` core that the implicit outcome-driven release paths (`ConfirmPublishOutcome.APPLY_FAILED`, `EmitTraceEvents` run-aborted) use; both paths coexist. Reason-code mapping (aligned with the EmitTraceEvents implicit path so audit reason values are consistent regardless of which release path the adapter took): `provider_error` / `runtime_error` / `client_timeout` → `RuntimeError`; `run_aborted` / `run_cancelled` → `RunAborted`; anything else → `Explicit`. Same-process idempotent replay returns the original `audit_event_signature` from a bounded sidecar replay cache; cache miss returns empty bytes rather than a fabricated signature. Replay is allowed to reach the ledger after local fencing TTL movement, while first-time release mutations still require active fencing. Ledger `IdempotencyConflict` uses a dedicated shared proto code and maps to gRPC `FailedPrecondition`. This closes [GH #85](https://github.com/m24927605/agentic-spendguard/issues/85), [GH #86](https://github.com/m24927605/agentic-spendguard/issues/86), and [GH #87](https://github.com/m24927605/agentic-spendguard/issues/87). The richer ledger-tier `ReleaseRequest` (with `reservation_set_id`, `Idempotency`, `Fencing`, `audit_event`, `decision_id`, `producer_sequence`) stays internal to the sidecar; adapters see only the Draft-01-shaped wire surface. See proto: `proto/spendguard/sidecar_adapter/v1/adapter.proto::ReleaseReservation`.
