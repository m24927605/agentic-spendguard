...
|-----------------------------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|-------------------------------------------------------------------------------------------------------------------------------------------|
| `spendguard-tls`                  | `ca.crt`, `ledger.crt`, `ledger.key`, `canonical-ingest.crt`, `canonical-ingest.key`, `control-plane.crt`, `control-plane.key`, `sidecar.crt`, `sidecar.key`, `webhook-receiver.crt`, `webhook-receiver.key`, `outbox-forwarder.crt`, `outbox-forwarder.key`, `ttl-sweeper.crt`, `ttl-sweeper.key` | mTLS workload certs. Production: provision via cert-manager + a workload-identity ClusterIssuer so rotation is automatic.                |
...
| `spendguard-postgres-urls`        | `ledger-url`, `canonical-url`, `control-plane-url`, `control-plane-audit-forwarder-url`, `tokenizer-url`, `output-predictor-url`, `output-predictor-plugin-endpoint-url`, `run-cost-projector-url`, `stats-aggregator-url`                                                                        | Database URLs contain credentials and are referenced only through Secret keys in rendered manifests.                                      |
...
| `spendguard-signing-keys`         | `ledger.pem`, `sidecar.pem`, `webhook-receiver.pem`, `ttl-sweeper.pem`, `control-plane.pem` (ed25519 private keys, PKCS8 PEM)                                                                                                                                                       | Per-producer signing keys for the audit chain. KMS-backed mode (S6) replaces this with `signing.kms.kmsKeyArn` per service.              |
...
kubectl create secret generic spendguard-postgres-urls \
  ...
  --from-literal=control-plane-audit-forwarder-url="$SPENDGUARD_CONTROL_PLANE_AUDIT_FORWARDER_DATABASE_URL" \
