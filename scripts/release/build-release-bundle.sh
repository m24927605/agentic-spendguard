...
Release image SBOM and provenance attestations are generated by
`.github/workflows/publish-images.yml` for pushed image digests. Local release
signoff is produced by:

```bash
scripts/security/ga-security-scan.sh --require-external-tools
```

That command writes Cargo dependency SBOM evidence, vulnerability scan output,
tool versions, and GA security invariant checks under
`docs/reviews/ga-readiness/GA_09_security_signoff_supply_chain/`.
SBOM
