//! D16 — Genspark importer binary entrypoint.
//!
...
//! * `live` — pull the Genspark admin API directly. Requires
//!   `GENSPARK_API_TOKEN`. Only compiled when the `live` Cargo feature
//!   is on; the default-feature binary errors out with a clear message.
//!
...
use spendguard_importer_genspark::{CloudEventEnvelope, CreditPriceTable, FixtureLoader};
...
fn print_help() {
    println!(
        "spendguard importer-genspark — Genspark (Super Agent) billing importer

USAGE:
    importer_genspark --mode fixture --fixture <PATH> [--tenant <ID>] [--budget <ID>]
    importer_genspark --mode live    [--tenant <ID>] [--budget <ID>]   # requires GENSPARK_API_TOKEN

OPTIONS:
    --mode <fixture|live>   Operating mode (default: fixture)
    --fixture <PATH>        Path to sanitized genspark_usage.json snapshot
...
fn run_fixture(args: &CliArgs) -> Result<(), String> {
    ...
    let prices = CreditPriceTable::load_from_embedded();
    ...
    for rec in loader.records() {
        ...
        let env = spendguard_importer_genspark::cloudevent_envelope::build(&rec, &prices)
    ...
    println!("{json}");
    eprintln!(
        "[importer_genspark] emitted {n} envelope(s) from fixture {p:?}",
        n = envelopes.len(),
        p = path,
    );
    Ok(())
...
fn run_live(_args: &CliArgs) -> Result<(), String> {
    Err("live mode binary scaffold not wired in this slice; \
         see services/importer_genspark/src/live/* for the client API"
        .to_string())
}
...
fn run_live(_args: &CliArgs) -> Result<(), String> {
    Err(
        "binary was built without the `live` Cargo feature; rebuild with \
         `cargo build -p spendguard-importer-genspark --features live`"
            .to_string(),
    )
}
