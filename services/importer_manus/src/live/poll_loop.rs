//! D15 COV_73 — bounded exponential-backoff poll loop. Review-standards
//! T12: per-request timeout ≤ 30s; backoff cap ≤ 1 hour to prevent
//! unbounded growth that could DoS the Manus admin REST API.
...
pub struct PollConfig {
    ...
    /// Upper bound for the backoff. Review-standards T12: ≤ 1 hour.
...
/// One synchronous cycle of the poll loop. Returns the records on
/// success, or `LiveError` on failure. Caller is responsible for
/// looping + applying `next_backoff`.
pub async fn run_poll_cycle(
    client: &super::client::ManusClient,
    window: (chrono::DateTime<chrono::Utc>, chrono::DateTime<chrono::Utc>),
) -> Result<Vec<crate::record::ImportRecord>, super::errors::LiveError> {
    client.poll_usage(window.0, window.1).await
}
...
mod tests {
    ...
    #[test]
    fn t12_backoff_cap_is_at_most_one_hour() {
        let cfg = PollConfig::default();
