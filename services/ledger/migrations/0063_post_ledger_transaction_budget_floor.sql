-- post_ledger_transaction budget floor (addresses Critical fail-open finding).
--
-- Finding: the main reserve path (post_ledger_transaction) never enforced a
-- non-negative `available_budget` floor. Per-unit balance assertion only
-- checks SUM(debit) == SUM(credit) within the tx; it does NOT reject a
-- reserve that drives the cumulative `available_budget` account balance
-- below zero. The ledger is documented as the single source of truth and
-- hard cap (Contract DSL §contract-dsl-spec-v1alpha2.md:138 — BUDGET_EXHAUSTED
-- fires "在 ledger reservation 階段 (hard cap)"), but the only thing actually
-- preventing over-spend was the sidecar's read-before-reserve. Any sidecar
-- bug, projection lag, or two reservations that individually fit but jointly
-- exceed the budget would silently over-reserve real money.
--
-- Fix: ADD a new step that, after the entries are inserted and under the
-- per-account FOR UPDATE row locks already taken in step 6, computes the
-- CUMULATIVE post-write `available_budget` balance for every available_budget
-- account this tx touched with a NET DEBIT (i.e. reserve ops), and RAISES
-- BUDGET_EXHAUSTED when any such balance would go negative. This mirrors
-- post_session_reserve's `v_available_balance < v_amount` denial
-- (0062:625-631) and makes the ledger — not the sidecar — the authority for
-- the hard cap.
--
-- Orientation (must match query_budget_state.rs:114-126 and
-- session_account_balance in 0062): `available_budget` is CREDIT-POSITIVE, so
-- balance = SUM(credit) - SUM(debit). A reserve debits available_budget
-- (lowers balance); release / refund / control-plane adjustment ops CREDIT
-- available_budget (raise balance) and therefore never trip the floor — the
-- check is scoped to accounts whose net effect IN THIS TX is a debit.
--
-- Funding model (verified): budgets are PRE-FUNDED into available_budget via
-- an operator opening deposit that CREDITS available_budget (see
-- deploy/demo/init/migrations/30_seed_demo_state.sh:251 "credit
-- available_budget 500"). So the `balance >= 0` floor never fires on a normal
-- under-budget reserve; it only fires when reserves would exceed the funded
-- ceiling.
--
-- This is a CREATE OR REPLACE with the IDENTICAL signature and GRANT set as
-- 0012; it does not alter any historical migration in place.

-- Helper: assert the cumulative `available_budget` floor for a posted tx.
-- For every (account) this tx touches with NET DEBIT on available_budget,
-- compute the cumulative balance across ALL committed entries for that
-- account (credit-positive orientation) and RAISE if any is negative.
CREATE OR REPLACE FUNCTION assert_available_budget_floor(p_tx_id UUID)
RETURNS VOID AS $$
DECLARE
    v_exhausted TEXT;
BEGIN
    SELECT string_agg(
               acct.ledger_account_id::TEXT || ':' || acct.balance::TEXT,
               ', ' ORDER BY acct.ledger_account_id)
      INTO v_exhausted
      FROM (
          -- available_budget accounts this tx wrote to with a NET DEBIT
          -- (reserve). Release / refund / adjustment ops net-credit these
          -- accounts and are intentionally excluded.
           ...
           WHERE ledger_transaction_id = p_tx_id
             AND ledger_account_id IN (
                     SELECT ledger_account_id
                      ...
                      WHERE account_kind = 'available_budget'
                 )
          ...
          HAVING SUM(
                     ...
                     END
                 ) > 0   -- net debit (debit-positive here) => available_budget went DOWN
      ) touched
      JOIN LATERAL (
          -- Cumulative balance for the touched account across ALL committed
          -- entries (prior reservations included — that is the source of
          -- over-spend). Credit-positive orientation: SUM(credit) - SUM(debit).
          SELECT touched.ledger_account_id,
                 COALESCE(SUM(
                     CASE direction
                         WHEN 'credit' THEN amount_atomic
                         WHEN 'debit'  THEN -amount_atomic
            ...
            FROM ledger_entries
           WHERE ledger_account_id = touched.ledger_account_id
      ) acct ON TRUE
     WHERE acct.balance < 0;

    IF v_exhausted IS NOT NULL THEN
        RAISE EXCEPTION
            'BUDGET_EXHAUSTED: available_budget would go negative for account(s): %',
            v_exhausted
            USING ERRCODE = 'P0001';
    END IF;
END;
...
BEGIN
    ...
    -- =========================================================
    -- 10b) HARD CAP: non-negative available_budget floor (BUDGET_EXHAUSTED).
    --      Authority for the spend hard cap lives HERE in the ledger, not the
    --      sidecar. Scoped to available_budget accounts this tx net-debits
    --      (reserve); release / refund / control-plane credits never trip it.
    --      Runs under the step-6 FOR UPDATE locks, so the cumulative post-write
    --      balance is read under the correct serialization (two reserves that
    --      individually fit but jointly exceed the funded budget => the second
    --      sees the first's debit and is DENIED).
    -- =========================================================
    PERFORM assert_available_budget_floor(v_tx_id);
...
-- =========================================================
-- Grants (identical to 0012 — re-asserted because CREATE OR REPLACE does
-- not change the existing grant set, but the new helper needs EXECUTE).
-- =========================================================
GRANT EXECUTE ON FUNCTION post_ledger_transaction(JSONB, JSONB, JSONB, JSONB, TEXT)
    TO ledger_application_role;
GRANT EXECUTE ON FUNCTION assert_available_budget_floor(UUID)
    TO ledger_application_role;
