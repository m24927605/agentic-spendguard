-- =====================================================================
-- 0034: Allow decision.producer_id = 'ledger:*' for server-minted rows
--       (Codex round-11 P2)
-- =====================================================================
--
-- Post-S6 invoice_reconcile mints the decision row with the ledger's
-- own producer signer; the canonical_ingest verifier
-- (services/canonical_ingest/src/verifier.rs:57) routes JSON-canonical
-- verification only when `producer_id.starts_with("ledger:")`. This
-- commit's invoice_reconcile.rs change sets
--   decision_payload.producer_id = "ledger:invoice-reconcile"
-- so the verifier picks the JSON path that matches the ledger's
-- signing input.
--
-- The SP's pre-existing cross-row metadata check (Δ11.7) required
-- decision.producer_id == outcome.producer_id, which was correct in
-- the pre-S6 stub world where both rows shared a producer. After
-- this change they're explicitly asymmetric:
--
--   * outcome.producer_id  = original caller (sidecar / webhook)
--   * decision.producer_id = "ledger:invoice-reconcile"
--
-- Migration 0034 replaces the equality check with: decision MUST
-- start with "ledger:". Other Δ11.7 cross-row checks (runid, source,
-- time, schema_bundle_id) stay as-is since those are still expected
-- to match between the two payloads.
--
-- Body copied from 0033 (the most recent SP definition).
-- emptiness check.
...
BEGIN
    ...
    -- =========================================================
    -- Codex round-11 P2: post-S6 the decision row is server-minted by
    -- the ledger and MUST carry a 'ledger:*' producer_id so the
    -- canonical_ingest verifier picks the JSON canonical-form path.
    -- The outcome row keeps its original caller producer_id, so
    -- equality between the two no longer holds. Replace the strict
    -- equality with: decision.producer_id LIKE 'ledger:%'. Other
    -- cross-row metadata still has to match between the rows.
    BEGIN
        IF      NOT ((v_dec_pl->>'producer_id') LIKE 'ledger:%')
