-- =====================================================================
-- 0031: Relax invoice_reconcile decision payload signing_key_id pin
--       (Codex round-7 P1)
-- =====================================================================
--
-- 0016's post_invoice_reconciled_transaction() pinned the decision
-- payload's signing_key_id to the legacy POC sentinel
-- 'ledger-server-mint:v1'. That sentinel was correct when the ledger
-- minted decisions with a hardcoded stub key. Phase 5 S6 introduced
-- real producer signers (LocalEd25519 / KMS / Disabled) and
-- invoice_reconcile (services/ledger/src/handlers/invoice_reconcile.rs:255)
-- now writes the *actual* signer's key_id into the synthesized
-- decision payload. Every InvoiceReconcile call therefore raises
-- 'AUDIT_INVARIANT_VIOLATED: decision payload literal mismatch' and
-- the audit rows are never written.
--
-- Fix: drop the literal pin. Structural invariants 0016 lines
-- 369 + 390 already enforce signing_key_id is a non-empty string;
-- key validity itself is enforced downstream by the canonical_ingest
-- verifier (S8). Keeping a hardcoded literal in the SP only models
-- the pre-S6 stub.
--
-- This migration CREATE OR REPLACE FUNCTIONs the SP. Body is
