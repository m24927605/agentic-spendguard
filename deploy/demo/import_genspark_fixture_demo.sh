#!/usr/bin/env bash
# =====================================================================
# D16 COV_88 — `demo-verify-import-genspark-fixture` runner.
# =====================================================================
#
# Self-contained demo for the Genspark billing importer's fixture flow.
# The importer is reconciliation-only (Genspark runs entirely inside
# Genspark's cloud VM — SpendGuard cannot gate), so the demo here is
# narrow:
#
#   1. Spin up a throwaway postgres:16-alpine container.
#   2. Apply the minimal schema + mig 0061 (D16 widen).
#   3. Run `cargo run -p spendguard-importer-genspark` against the
...
#   5. Run `verify_step_import_genspark_fixture.sql` against the
#      container's postgres.
#   6. Clean up the container on exit.
#
# The runner is idempotent: INSERTs use ON CONFLICT DO NOTHING keyed on
# (event_id) — the deterministic UUIDv5 makes re-runs land zero new
# rows.
#
# Exit codes:
...
CRATE_DIR="$REPO_ROOT/services/importer_genspark"
FIXTURE_PATH="$CRATE_DIR/tests/fixtures/genspark_usage.json"
VERIFY_SQL="$REPO_ROOT/deploy/demo/verify_step_import_genspark_fixture.sql"

CONTAINER_NAME="${SPENDGUARD_D16_PG_CONTAINER:-spendguard-d16-import-genspark-pg}"
PG_USER="spendguard"
PG_PASSWORD="spendguard_demo"
PG_DB="spendguard_ledger"
PG_HOST_PORT="${SPENDGUARD_D16_PG_PORT:-5441}"   # avoid colliding with the main demo on :5433 and D14 on :5439

# --------------------------------------------------------------------
# Step 0 — preflight
# --------------------------------------------------------------------
command -v docker >/dev/null 2>&1 || fail "docker not on PATH; required for demo postgres"
command -v cargo  >/dev/null 2>&1 || fail "cargo not on PATH; required to build importer_genspark"
...
# Run psql INSIDE the container so we don't depend on a host-side psql.
...
CREATE TABLE IF NOT EXISTS audit_outbox (
    ...
    model              TEXT         NULL,
    credits_consumed   DOUBLE PRECISION NULL,
...
# --------------------------------------------------------------------
# Step 3 — apply the D16 mig 0061 widening (idempotent)
# --------------------------------------------------------------------
log "applying mig 0061 (D16 audit_outbox.import_source widen)"
$PSQL <<'SQL'
...
ENVELOPES_JSON="$(cargo run --quiet --manifest-path "$CRATE_DIR/Cargo.toml" --bin importer_genspark -- \
    --mode fixture \
    --fixture "$FIXTURE_PATH" \
    --tenant demo \
    --budget genspark-budget 2>/dev/null)"
...
[ "$ENV_COUNT" -ge 3 ] || fail "expected >= 3 envelopes; got $ENV_COUNT"

# --------------------------------------------------------------------
# Step 5 — translate envelopes → audit_outbox INSERTs
# --------------------------------------------------------------------
log "inserting audit_outbox rows from envelopes"
INSERT_SQL="$(printf '%s' "$ENVELOPES_JSON" | python3 "$REPO_ROOT/deploy/demo/import_genspark_fixture_emit_sql.py")"
...
# --------------------------------------------------------------------
ROWCOUNT_BEFORE="$(docker exec -i "$CONTAINER_NAME" psql -At -U "$PG_USER" -d "$PG_DB" -c "SELECT COUNT(*) FROM audit_outbox WHERE tenant_id='demo' AND import_source='genspark_team_api';")"
printf '%s' "$INSERT_SQL" | $PSQL >/dev/null
ROWCOUNT_AFTER="$(docker exec -i "$CONTAINER_NAME" psql -At -U "$PG_USER" -d "$PG_DB" -c "SELECT COUNT(*) FROM audit_outbox WHERE tenant_id='demo' AND import_source='genspark_team_api';")"
...
log "D16 import_genspark_fixture demo verified — exiting 0"
