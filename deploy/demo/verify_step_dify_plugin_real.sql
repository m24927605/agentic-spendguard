-- D10 SLICE 7 (dify_plugin_real demo) — ledger-DB assertions.
--
-- Mirrors verify_step_langchain_ts.sql / verify_step_kong_gateway_real.sql /
-- verify_step_vercel_ai_mastra.sql: ledger-side gates only. The Makefile
-- target `demo-verify-dify-plugin-real` runs this file against
-- spendguard_ledger.
--
-- D10 review-standards §7 gates:
--   - reserve >= 2 (ALLOW + STREAM each produce a reservation)
--   - commit_estimated >= 2 (both ALLOW paths commit real usage)
...
-- earliest reserve row predates the earliest LLM_CALL_POST outcome row.

\echo
\echo === ledger_transactions: operation_kind counts (dify_plugin_real) ===
...
BEGIN
    ...
    IF v_reserve < 2 THEN
        RAISE EXCEPTION 'D10_GATE: ledger_transactions.reserve >= 2 expected (ALLOW + STREAM), got %', v_reserve;
    END IF;
    IF v_commit < 2 THEN
        RAISE EXCEPTION 'D10_GATE: ledger_transactions.commit_estimated >= 2 expected (ALLOW + STREAM), got %', v_commit;
    END IF;
    IF v_denied < 1 THEN
        RAISE EXCEPTION 'D10_GATE: ledger_transactions.denied_decision >= 1 expected (DENY step), got %', v_denied;
    END IF;

    RAISE NOTICE 'D10 LEDGER OK: reserve=% commit_estimated=% denied_decision=%',
...
BEGIN
    ...
    IF v_first_reserve IS NULL THEN
        RAISE EXCEPTION 'D10_INV2_GATE: no reservations recorded';
    END IF;
    IF v_first_outcome IS NULL THEN
        RAISE EXCEPTION 'D10_INV2_GATE: no outcome events recorded';
    END IF;
    IF v_first_outcome < v_first_reserve THEN
        RAISE EXCEPTION 'D10_INV2_GATE: outcome before reserve (% < %); INV-2 violated',
            v_first_outcome, v_first_reserve;
    END IF;
    RAISE NOTICE 'D10 INV-2 OK: first_reserve=% first_outcome=%',
