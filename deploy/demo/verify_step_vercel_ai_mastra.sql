-- COV_D06 SLICE 7 (vercel_ai_mastra demo) — ledger-DB assertions.
--
-- Mirrors verify_step_langchain_ts.sql: ledger-side gates only.
-- The Makefile target `demo-verify-vercel-ai-mastra` runs this file
...
--     before the upstream HTTP call leaves the vercel-ai-mastra-runner
--     process)
--
...
\echo
\echo === ledger_transactions: operation_kind counts (vercel_ai_mastra) ===
...
BEGIN
    ...
    IF v_reserve < 2 THEN
        RAISE EXCEPTION 'COV_D06_GATE: ledger_transactions.reserve >= 2 expected (ALLOW + STREAM), got %', v_reserve;
    END IF;
    IF v_commit < 2 THEN
        RAISE EXCEPTION 'COV_D06_GATE: ledger_transactions.commit_estimated >= 2 expected (ALLOW + STREAM), got %', v_commit;
    END IF;
    IF v_denied < 1 THEN
        RAISE EXCEPTION 'COV_D06_GATE: ledger_transactions.denied_decision >= 1 expected (DENY step), got %', v_denied;
    END IF;

    RAISE NOTICE 'COV_D06 LEDGER OK: reserve=% commit_estimated=% denied_decision=%',
...
BEGIN
    ...
    IF v_first_reserve IS NULL THEN
        RAISE EXCEPTION 'COV_D06_GATE: no reservation rows found for INV-2 check';
    END IF;
    IF v_first_outcome IS NULL THEN
        RAISE EXCEPTION 'COV_D06_GATE: no outcome rows found for INV-2 check';
    END IF;
    IF v_first_reserve >= v_first_outcome THEN
        RAISE EXCEPTION 'COV_D06_GATE: INV-2 violated — first reserve=% NOT before first outcome=%',
            v_first_reserve, v_first_outcome;
    END IF;
    RAISE NOTICE 'COV_D06 INV-2 OK: first_reserve=% < first_outcome=%',
        v_first_reserve, v_first_outcome;
END;
$$;

\echo
\echo === ASSERT: audit_outbox carries vercel-ai decision rows ===
-- The @spendguard/vercel-ai middleware's transformParams() emits
-- decisions via the shared SpendGuardClient. The SLICE 3 wire shape
-- does not currently thread a `runtime_kind` field through the
-- wire surface (deferred to a later slice; tests cover the
-- locked-surface contract). For now we assert decision-row presence
-- by the demo-mode tenant + recency window; the runner-side `[demo]
-- vercel_ai_mastra ALL 3 steps PASS` line gives the upstream
-- causal-ordering signal.
DO $$
DECLARE
    v_decision INT;
BEGIN
    ...
    IF v_decision < 2 THEN
        RAISE EXCEPTION 'COV_D06_GATE: audit_outbox decision rows >= 2 expected (ALLOW + DENY both emit decisions), got %', v_decision;
    END IF;
    RAISE NOTICE 'COV_D06 AUDIT OK: decision rows=%', v_decision;
END;
$$;
