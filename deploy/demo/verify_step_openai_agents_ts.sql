-- COV_D08 SLICE 5 (openai_agents_ts demo) — ledger-DB assertions.
--
-- Mirrors verify_step_langchain_ts.sql: ledger-side gates only.
-- The Makefile target `demo-verify-openai-agents-ts` runs this file
...
--     before the OpenAI HTTP call leaves the openai-agents-runner process)
--
...
\echo
\echo === ledger_transactions: operation_kind counts (openai_agents_ts) ===
...
BEGIN
    ...
    IF v_reserve < 2 THEN
        RAISE EXCEPTION 'COV_D08_GATE: ledger_transactions.reserve >= 2 expected (ALLOW + STREAM), got %', v_reserve;
    END IF;
    IF v_commit < 2 THEN
        RAISE EXCEPTION 'COV_D08_GATE: ledger_transactions.commit_estimated >= 2 expected (ALLOW + STREAM), got %', v_commit;
    END IF;
    IF v_denied < 1 THEN
        RAISE EXCEPTION 'COV_D08_GATE: ledger_transactions.denied_decision >= 1 expected (DENY step), got %', v_denied;
    END IF;

    RAISE NOTICE 'COV_D08 LEDGER OK: reserve=% commit_estimated=% denied_decision=%',
...
BEGIN
    ...
    IF v_first_reserve IS NULL THEN
        RAISE EXCEPTION 'COV_D08_GATE: no reservation rows found for INV-2 check';
    END IF;
    IF v_first_outcome IS NULL THEN
        RAISE EXCEPTION 'COV_D08_GATE: no outcome rows found for INV-2 check';
    END IF;
    IF v_first_reserve >= v_first_outcome THEN
        RAISE EXCEPTION 'COV_D08_GATE: INV-2 violated — first reserve=% NOT before first outcome=%',
            v_first_reserve, v_first_outcome;
    END IF;
    RAISE NOTICE 'COV_D08 INV-2 OK: first_reserve=% < first_outcome=%',
        v_first_reserve, v_first_outcome;
END;
$$;

\echo
\echo === ASSERT: audit_outbox carries openai-agents decision rows ===
DO $$
DECLARE
    v_decision INT;
BEGIN
    ...
    IF v_decision < 2 THEN
        RAISE EXCEPTION 'COV_D08_GATE: audit_outbox decision rows >= 2 expected (ALLOW + DENY both emit decisions), got %', v_decision;
    END IF;
    RAISE NOTICE 'COV_D08 AUDIT OK: decision rows=%', v_decision;
END;
$$;
