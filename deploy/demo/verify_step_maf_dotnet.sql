-- COV_d07 SLICE 8 (maf_dotnet_real demo) — ledger-DB assertions.
--
-- Mirrors verify_step_openai_agents_ts.sql: ledger-side gates only.
-- The Makefile target `demo-verify-maf-dotnet` runs this file against
...
--     before the inner IChatClient HTTP call leaves the
--     maf-dotnet-runner process)
--
...
\echo
\echo === ledger_transactions: operation_kind counts (maf_dotnet_real) ===
...
BEGIN
    ...
    IF v_reserve < 2 THEN
        RAISE EXCEPTION 'COV_D07_DOTNET_GATE: ledger_transactions.reserve >= 2 expected (ALLOW + ALLOW2), got %', v_reserve;
    END IF;
    IF v_commit < 2 THEN
        RAISE EXCEPTION 'COV_D07_DOTNET_GATE: ledger_transactions.commit_estimated >= 2 expected (ALLOW + ALLOW2), got %', v_commit;
    END IF;
    IF v_denied < 1 THEN
        RAISE EXCEPTION 'COV_D07_DOTNET_GATE: ledger_transactions.denied_decision >= 1 expected (DENY step), got %', v_denied;
    END IF;

    RAISE NOTICE 'COV_D07_DOTNET LEDGER OK: reserve=% commit_estimated=% denied_decision=%',
...
BEGIN
    ...
    IF v_first_reserve IS NULL THEN
        RAISE EXCEPTION 'COV_D07_DOTNET_GATE: no reservation rows found for INV-2 check';
    END IF;
    IF v_first_outcome IS NULL THEN
        RAISE EXCEPTION 'COV_D07_DOTNET_GATE: no outcome rows found for INV-2 check';
    END IF;
    IF v_first_reserve >= v_first_outcome THEN
        RAISE EXCEPTION 'COV_D07_DOTNET_GATE: INV-2 violated — first reserve=% NOT before first outcome=%',
            v_first_reserve, v_first_outcome;
    END IF;
    RAISE NOTICE 'COV_D07_DOTNET INV-2 OK: first_reserve=% < first_outcome=%',
        v_first_reserve, v_first_outcome;
END;
$$;

\echo
\echo === ASSERT: audit_outbox carries maf-dotnet decision rows ===
DO $$
DECLARE
    v_decision INT;
BEGIN
    ...
    IF v_decision < 2 THEN
        RAISE EXCEPTION 'COV_D07_DOTNET_GATE: audit_outbox decision rows >= 2 expected (ALLOW + DENY both emit decisions), got %', v_decision;
    END IF;
    RAISE NOTICE 'COV_D07_DOTNET AUDIT OK: decision rows=%', v_decision;
END;
$$;
