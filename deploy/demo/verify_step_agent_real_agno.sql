-- COV_D22 SLICE 4 (agent_real_agno demo) — ledger-DB assertions.
--
-- Mirrors verify_step_agent_real_strands.sql: ledger-side gates only.
-- The Makefile target `demo-verify-agent-real-agno` runs this file
...
-- decision_context_json->>'integration' = 'agno' tag.
--
...
-- PRE wraps DecisionDenied into an InputCheckError BEFORE Agno
-- dispatches the model HTTP call — verified by counting-stub hits
...
\echo
\echo === ledger_transactions: operation_kind counts (agent_real_agno) ===
...
BEGIN
    ...
    IF v_reserve < 1 THEN
        RAISE EXCEPTION 'COV_D22_GATE: ledger_transactions.reserve >= 1 expected (ALLOW), got %', v_reserve;
    END IF;
    IF v_commit < 1 THEN
...
BEGIN
    ...
    IF v_first_reserve IS NULL OR v_first_commit IS NULL THEN
        RAISE NOTICE 'COV_D22 INV-2: insufficient rows for strict-order check (reserve=%, commit=%)',
            v_first_reserve, v_first_commit;
    ELSIF v_first_reserve > v_first_commit THEN
        RAISE EXCEPTION 'COV_D22 INV-2 VIOLATED: earliest reserve % > earliest commit %',
            v_first_reserve, v_first_commit;
    ELSE
        RAISE NOTICE 'COV_D22 INV-2 OK: reserve % predates commit %',
            v_first_reserve, v_first_commit;
    END IF;
END;
$$;
...
\echo
\echo === COV_D22 SLICE 4 agent_real_agno verification done ===
