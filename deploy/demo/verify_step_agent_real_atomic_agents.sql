-- COV_D28 SLICE 4 (agent_real_atomic_agents demo) — ledger-DB assertions.
--
-- Mirrors verify_step_agent_real_autogen.sql / verify_step_agent_real_beeai.sql:
-- ledger-side gates only. The Makefile target
-- `demo-verify-agent-real-atomic-agents` runs this file against
-- spendguard_ledger, then issues a second
-- `psql -d spendguard_canonical` block for the cross-DB
-- canonical_events check (decision/outcome counts) + the
-- decision_context_json->>'integration' = 'atomic_agents' tag.
--
-- Review-standards §5.1 + acceptance §3 gates:
--   - reserve >= 1 (ALLOW path: SpendGuardInstructorProxy's gated raw
--     create_fn reserves via request_decision before the inner
...
-- path because the gated raw raises DecisionDenied BEFORE inner
-- transport — verified by counting-stub hits staying flat on the DENY
-- turn). Here in the ledger we complement with a DB-side assertion
-- that the EARLIEST reserve row in this demo run predates the
-- EARLIEST commit_estimated row.

\echo
\echo === ledger_transactions: operation_kind counts (agent_real_atomic_agents) ===
...
BEGIN
    ...
    IF v_reserve < 1 THEN
        RAISE EXCEPTION 'COV_D28_GATE: ledger_transactions.reserve >= 1 expected (ALLOW), got %', v_reserve;
    END IF;
    IF v_commit < 1 THEN
...
BEGIN
    ...
    IF v_first_reserve IS NULL OR v_first_commit IS NULL THEN
        RAISE NOTICE 'COV_D28 INV-2: insufficient rows for strict-order check (reserve=%, commit=%)',
            v_first_reserve, v_first_commit;
    ELSIF v_first_reserve > v_first_commit THEN
        RAISE EXCEPTION 'COV_D28 INV-2 VIOLATED: earliest reserve % > earliest commit %',
            v_first_reserve, v_first_commit;
    ELSE
        RAISE NOTICE 'COV_D28 INV-2 OK: reserve % predates commit %',
            v_first_reserve, v_first_commit;
    END IF;
END;
$$;
...
\echo
\echo === COV_D28 SLICE 4 agent_real_atomic_agents verification done ===
