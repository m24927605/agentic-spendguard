-- COV_D04 SLICE 5 (langchain_ts demo) — ledger-DB assertions.
--
-- Mirrors verify_step_envoy_extproc.sql: ledger-side gates only.
-- The Makefile target `demo-verify-langchain-ts` runs this file
...
--     before the OpenAI HTTP call leaves the langchain-runner process)
--
...
\echo
\echo === ledger_transactions: operation_kind counts (langchain_ts) ===
...
BEGIN
    ...
    IF v_reserve < 2 THEN
        RAISE EXCEPTION 'COV_D04_GATE: ledger_transactions.reserve >= 2 expected (ALLOW + STREAM), got %', v_reserve;
    END IF;
    IF v_commit < 2 THEN
        RAISE EXCEPTION 'COV_D04_GATE: ledger_transactions.commit_estimated >= 2 expected (ALLOW + STREAM), got %', v_commit;
    END IF;
    IF v_denied < 1 THEN
        RAISE EXCEPTION 'COV_D04_GATE: ledger_transactions.denied_decision >= 1 expected (DENY step), got %', v_denied;
    END IF;

    RAISE NOTICE 'COV_D04 LEDGER OK: reserve=% commit_estimated=% denied_decision=%',
...
BEGIN
    ...
    IF v_first_reserve IS NULL THEN
        RAISE EXCEPTION 'COV_D04_GATE: no reservation rows found for INV-2 check';
    END IF;
    IF v_first_outcome IS NULL THEN
        RAISE EXCEPTION 'COV_D04_GATE: no outcome rows found for INV-2 check';
    END IF;
    IF v_first_reserve >= v_first_outcome THEN
        RAISE EXCEPTION 'COV_D04_GATE: INV-2 violated — first reserve=% NOT before first outcome=%',
            v_first_reserve, v_first_outcome;
    END IF;
    RAISE NOTICE 'COV_D04 INV-2 OK: first_reserve=% < first_outcome=%',
        v_first_reserve, v_first_outcome;
END;
$$;

\echo
\echo === ASSERT: audit_outbox carries langchain-js decision rows ===
-- The @spendguard/langchain handler's reserve() emits decisions via
-- the shared SpendGuardClient. The SLICE 3 handler does not currently
-- thread a `runtime_kind` field through the wire shape (deferred to a
-- later slice; tests.md A4.2 covers the locked-surface contract). For
-- now we assert decision-row presence by the demo-mode tenant +
-- recency window; the runner-side `[demo] langchain_ts ALL 3 steps
-- PASS` line gives the upstream causal-ordering signal.
DO $$
DECLARE
    v_decision INT;
BEGIN
    ...
    IF v_decision < 2 THEN
        RAISE EXCEPTION 'COV_D04_GATE: audit_outbox decision rows >= 2 expected (ALLOW + DENY both emit decisions), got %', v_decision;
    END IF;
    RAISE NOTICE 'COV_D04 AUDIT OK: decision rows=%', v_decision;
END;
$$;
