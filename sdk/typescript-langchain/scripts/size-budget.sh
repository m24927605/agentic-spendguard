#!/usr/bin/env bash
# D04 SLICE 6 — npm pack size budget gate.
#
# Asserts the final published tarball stays below the 50 KB budget. The
# adapter is thin glue (handler.ts + options.ts + ids.ts + extract logic);
# the budget head-room covers the @langchain/core peer-import surface and a
# small amount of headroom for additive optional fields landing in v0.2.x
# (windowInstanceId / unit / pricing / claimEstimator — see CHANGELOG.md
# Known limitations).
#
# Budget: 50 KB (gzipped tarball). `npm pack` emits the gzipped tarball
# size as `.size` in --json output. The D04 design (`design.md` §slice 6
# title) calls 40 KB the design intent; 50 KB gives margin for the
# additive options surface without thrashing the gate.
#
# Mirrors `sdk/typescript/scripts/size-budget.sh` shape, with the budget
# value scoped down (D05 ships ~250 KB because it carries the
# proto-generated TypeScript declarations; D04 carries none).
...
# than a broken pipe.
TMP_JSON="$(mktemp -t spendguard-langchain-pack.XXXXXX.json)"
...
# gate (it includes the source maps which are themselves never published).
