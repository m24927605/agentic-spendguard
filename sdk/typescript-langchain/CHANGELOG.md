...
- `SpendGuardCallbackHandlerOptions.unitId` — optional canonical-truth UUID
  of the ledger unit row. When set, the handler threads it through to
  `BudgetClaim.unit.unitId` on the reserve + commit wire (closes the
  HARDEN_D05_UR substrate gap that previously blocked DENY+STREAM full
  assertion across ~14 adapter demos). Most operators source this from the
...
- Backward-compat: omitting `unitId` matches prior behavior (sends empty
  ...
  baseline test modified. Closes HARDEN_D05_UR_S02 for D04.
