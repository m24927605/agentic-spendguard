"""Options dataclasses for the AutoGen / AG2 SpendGuard adapter.

Parity with the Strands / ADK / Agno adapters: a thin POCO config
bundle is *optional* — the public wrapper class
``SpendGuardChatCompletionClient`` accepts the same fields as keyword
args. Use ``SpendGuardAutoGenOptions`` when wiring the adapter via a
config file or ``hydra`` / ``pydantic-settings``.
...
class SpendGuardAutoGenOptions:
    ...
    Attributes:
        ...
        route: ``request_decision.route``. Defaults to ``"llm.call"`` so
            ...
            DSPy / Agno integrations on the dashboard.
    ...
    def __post_init__(self) -> None:
        ...
        if not self.tenant_id or not self.tenant_id.strip():
            raise SpendGuardConfigError(
                "SpendGuardAutoGenOptions.tenant_id must be a non-empty string."
            )
        if not self.budget_id or not self.budget_id.strip():
            raise SpendGuardConfigError(
                "SpendGuardAutoGenOptions.budget_id must be a non-empty string."
            )
        if not self.window_instance_id or not self.window_instance_id.strip():
            raise SpendGuardConfigError(
                "SpendGuardAutoGenOptions.window_instance_id must be a "
                "non-empty string."
            )
        if not self.sidecar_socket_path or not self.sidecar_socket_path.strip():
            raise SpendGuardConfigError(
                "SpendGuardAutoGenOptions.sidecar_socket_path must be a "
                "non-empty string."
            )
