"""Options dataclass for the Atomic Agents + Instructor adapter.

Parity with the autogen / beeai / agno / strands / adk adapters: a
thin POCO config bundle is *optional* — the public
``wrap_instructor_client`` factory accepts the same fields as keyword
args. Use ``SpendGuardAtomicAgentsOptions`` when wiring the adapter
...
class SpendGuardAtomicAgentsOptions:
    ...
    Attributes:
        ...
        route: ``request_decision.route``. Defaults to ``"llm.call"`` so
            ...
            DSPy / Agno / BeeAI / AutoGen integrations on the dashboard.
    ...
    When provided, the Instructor client wrapper threads it through to
    ...
    def __post_init__(self) -> None:
        ...
        if not self.tenant_id or not self.tenant_id.strip():
            raise SpendGuardConfigError(
                "SpendGuardAtomicAgentsOptions.tenant_id must be a non-empty string."
            )
        if not self.budget_id or not self.budget_id.strip():
            raise SpendGuardConfigError(
                "SpendGuardAtomicAgentsOptions.budget_id must be a non-empty string."
            )
        if not self.window_instance_id or not self.window_instance_id.strip():
            raise SpendGuardConfigError(
                "SpendGuardAtomicAgentsOptions.window_instance_id must be a "
                "non-empty string."
            )
        if not self.sidecar_socket_path or not self.sidecar_socket_path.strip():
            raise SpendGuardConfigError(
                "SpendGuardAtomicAgentsOptions.sidecar_socket_path must be a "
                "non-empty string."
            )
