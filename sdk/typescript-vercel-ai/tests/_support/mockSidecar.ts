// D06 SLICE 6 — In-process mock of `SpendGuardClient` for the provider
...
// The Vercel AI SDK adapter only sees the `SpendGuardClient` *interface* —
// `transformParams` / `wrapGenerate` / `wrapStream` consume `reserve()` and
// `commitEstimated()` directly. Mirroring the D04 SLICE 4 langchain mock
// keeps the SLICE 6 provider matrix focused on the middleware ↔ provider
// surface (the rest of the substrate is exercised by D05's own UDS suite).
//
// Per-call axes the mock surfaces (matched against review-standards §7):
//
//   1. `nextDecision`  — happy-path ALLOW or terminal DENY / STOP /
//      APPROVAL_REQUIRED.
//   2. `simulatedLatencyMs` — sleep before resolving so concurrency tests
//      can prove independent inflight slots.
//   3. `simulatedReserveError` — reject `reserve()` with a typed error so
//      the middleware's throw-on-deny vs swallow-on-unavailable path is
//      exercised.
//   4. `simulatedCommitError`  — reject `commitEstimated()` so the
//      middleware's commit-error swallow path is exercised.
//
// The mock is plain TypeScript — no UDS, no gRPC, no protobuf — so the
 ...
 * DENY / STOP / APPROVAL_REQUIRED → rejects with the matching typed error
 * from `@spendguard/sdk`.
 *
 * `SIDECAR_UNAVAILABLE` is also surfaced as a `DecisionPlan` variant — it is
 * the canonical operational-degradation case that the middleware MUST
 * SWALLOW (the LLM call proceeds without a budget gate). Keeping it in the
 * same enum as ALLOW/DENY means tests pick the desired path with a single
 * field.
 */
 ...
 * wire-shape fields the middleware emitted (`trigger`, `runId`, `llmCallId`,
...
/**
 * Snapshot of one `commitEstimated` invocation. Mirrors `RecordedReserve`
 * for the POST/ERROR branch — `outcomeKind` carries SUCCESS / FAILURE so
 * tests can assert wrapGenerate vs wrapStream commit shapes without
 * re-deriving from the `outcome` field.
 */
 ...
 * when the queue empties it falls back to the default ALLOW. Mirrors the
 * D04 SLICE 4 shape exactly — concurrent-invokes scenarios queue
 * distinct decisions for each parallel call.
 */
...
/**
 * In-process mock implementing the `SpendGuardClient` interface surface
 * the Vercel AI SDK adapter touches. Construction is synchronous (no UDS
 * bind, no gRPC channel), so tests start instantly.
 *
 * Usage:
 *
 ...
 *     });
 *     // ... drive a wrapLanguageModel(...) through the middleware ...
 ...
 *   - `client` — cast to `SpendGuardClient` so the middleware can consume
 *     it.
 */
export class MockSpendGuardClient {
   ...
   * implements the surface the SLICE 3-5 reserve/commit path touches.
   */
  ...
  /** Convenience: the last reserve request the middleware emitted. */
  get lastReserveRequest(): ReserveRequest | undefined {
    return this.reserveCalls[this.reserveCalls.length - 1]?.request;
  }

  /** Convenience: the last commit request the middleware emitted. */
 ...
 * middleware's default heuristic.
 */
