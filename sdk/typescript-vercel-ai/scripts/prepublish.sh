#!/usr/bin/env bash
# D06 SLICE 8 — `prepublishOnly` script.
#
...
#      AND the `dist/mastra.js` Mastra subpath alias entry, both with
#      matching `.d.ts` declarations.
#
# Mirrors `sdk/typescript-langchain/scripts/prepublish.sh` field-for-field.
#
